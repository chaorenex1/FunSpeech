...
class VoiceManager:
    ...
    @staticmethod
    def _format_cosyvoice3_prompt_text(reference_text: str = "") -> str:
        """格式化传给 CosyVoice3 add_zero_shot_spk 的参考文本。
    ...
    def _format_reference_text_for_model(self, reference_text: str) -> str:
        """CosyVoice3 在注册音色时需要把系统 prompt 拼入 reference_text。"""
        if self._is_cosyvoice3():
            return self._format_cosyvoice3_prompt_text(reference_text)
    ...
    def add_voice(self, voice_name: str, txt_file: Path, wav_file: Path) -> bool:
        ...
        try:
            ...
            model_reference_text = self._format_reference_text_for_model(reference_text)
            success = cosyvoice.add_zero_shot_spk(
                model_reference_text, str(wav_file), voice_name
            )
